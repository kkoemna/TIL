...
### ターミナルから数値を入力する
ターミナルから数値を入力するには`gets`というメソッドを使用する。

しかし、この`gets`メソッドを使って入力した値は全て文字列になる。<br>
そのため、入力された値を数値として扱いたい場合は、`to_i`メソッドを使って数値に変換する。
...
### 条件分岐
条件分岐はif文を使用する。今回は3つのパターンで条件を分けるため、elsifとelseも使う。
...
if文は先に書いた条件から判断していく。そのため上の条件に当てはまったら、たとえ下の条件に当てはまっても、下の条件の処理はスルーされる。

よって、下の条件と条件範囲が丸かぶりしないように、順番を考慮する必要がある。

# 13.eachの入れ子
## 問題
以下のように、果物の名前と値段が入った配列があります。<br>
...
## 解説
最初に、3行目でfruits_priceから`["apple", [200, 250, 220]]`という値を取り出し、変数fruitに代入する。その後4行目〜7行目で、fruitの1番目の値`[200, 250, 220]`から値を1つずつ取り出して、自己代入しながらsumを出力する。詳しくは以下のとおり。
...
この結果、`appleの合計金額は670円です`と出力されるようになる。この作業を残り2つorange、melonでも繰り返す。
