...
# バリデーション
バリデーションは、入力情報を検証するための仕組み。コントローラの`validate`メソッドを利用する。

```php
this->validate($request, [ 検証設定の配列] );
```

第1引数にはリクエストを、第2引数には検証する設定情報を配列にまとめたものを用意する。設定情報は`〇〇という項目には××の検証をする`というように、フォームのコントロール名とそこに割り当てる検証ルールで成り立っている。
...
## メッセージのカスタマイズ
バリデーションのエラーメッセージをデフォルト表示から任意のメッセージに変更するには、FormRequestのmessagesメソッドに記述する。

```
'項目名.ルール名' => 'メッセージ'
```

という形で以下のようにメッセージ情報を配列にまとめる。
...
## バリデータ
バリデーションの結果によって処理を分岐させたい場合には、バリデータを利用する。
